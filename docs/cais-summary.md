...
Ghost Decisions itself runs on keyword/topic overlap, not semantic understanding, by design choice rather than oversight: an embedding-similarity upgrade was built (`core/ghost/preventive.py` accepts optional decision/diff embeddings) and evaluated before being wired into the live gate. A dry-run found true-positive and false-positive similarity scores (0.29 vs. 0.65) don't separate cleanly — shared surface vocabulary between unrelated decisions and diffs produces false-positive similarity as high as genuine matches. It was deliberately not shipped to the live path rather than shipping a detector that doesn't reliably discriminate.

**An LLM-based intent check (#106, `core/ghost/intent_check.py`) was built and live-verified on 2026-09-01** as that next candidate, against a real OpenAI backend rather than a mock: given a decision requiring authentication before payment processing and a diff that removes exactly that check, it correctly returned a conflict with a specific rationale; given an unrelated documentation diff against the same decision, it correctly returned no conflict. It's wired into the live `ghost_check` endpoint (`core/ghost/preventive_router.py`), advisory-only and deliberately narrow — it only fires on an *existing* medium-severity keyword warning against a high/critical-risk decision, escalating that warning to high rather than replacing keyword matching or running against every diff. Both the cache (keyed by decision+diff) and the per-project daily rate limit (50 calls/day by default) are persisted in project memory, not in-process, so they survive a restart. It does **not** move Drift-Bench's reward-hacking or multi-step-drift cells: both of those scenarios are built around a diff with *zero* keyword overlap against the violated decision, so Ghost produces no warning at all there for #106 to escalate — confirmed by running the real `reward_hacking_keyword_evasion` decision/diff pair through `check_diff_for_warnings` directly (empty result, not assumed). #106 closes a narrower, different gap than either open cell: weak-but-nonzero overlap on a high-risk decision that got scored down to medium, not the zero-overlap keyword-evasion or session-memory gaps documented below.
