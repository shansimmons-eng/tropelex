...
**Status:** Open. Proposed 2026-08-07, extended 2026-08-10. The single most actionable item from the agent-drift research pass — cheapest to build, most directly matches the "ongoing, not set-and-forget" philosophy behind #35/Needs Attention.
...
**Why:** Drift-Bench (#60) just measured this exact gap directly, not theoretically — the reward-hacking scenario (a diff adding `if user.email == "debug@internal.test": return True` against a decision "Never bypass authentication for admin-level access") shares zero keywords with the decision and correctly, honestly scores 0% detection. Verified against the actual code: `core/ghost/pattern_matcher.py`/`preventive.py` have zero `embed`/`hybrid_similarity` usage (confirmed via direct grep) — Ghost is pure Jaccard keyword overlap. Meanwhile #57 already built and proved the exact infrastructure this needs: `core/contradictions/detector.py`'s `hybrid_similarity(text_a, text_b, embedding_a, embedding_b)` blends keyword + cosine similarity, `core/llm.py`'s `embed`/`embed_one` already generate the vectors (gracefully `None` with no `OPENAI_API_KEY`), and `core/contradictions/router.py`'s `_get_decision_embeddings` already caches them per-project. This is extending a proven pattern to a second detector, not inventing one — but #57's own incident (round 1: hybrid scoring against thresholds tuned for pure-keyword's sparse distribution jumped a live project from 8 to 61 pending reviews before the bug was caught) is the load-bearing lesson here: any threshold work must be dry-run against the real `tropelex` project before touching a live escalation path, not assumed safe from the design alone.

**Features (proposed):**
- Extend `match_decision_to_diff`/`score_ghost_severity` (`core/ghost/pattern_matcher.py`) to accept optional decision/diff embeddings, blending semantic similarity into the overlap score the same way `hybrid_similarity` does for Contradictions — same dual-gate discipline #57 landed on (permissive semantic gate can rescue a real match with low keyword overlap; a pure-keyword signal stays the strict/conservative path), not a wholesale threshold replacement.
- Optional, budget-controlled LLM check ("does this diff make the decision's claim false?") gated to high-severity/high-risk decisions only — mirrors #54's already-established "richer checks only above a risk threshold" pattern rather than running an LLM call on every diff.
- Wire the new signal into Drift-Bench (#60): the `reward_hacking_keyword_evasion` scenario should start scoring as a true positive once this ships, closing the exact gap #60 published.
- **Must dry-run against the real `tropelex` project before wiring into #53's enforcement gate** — same discipline #57 was forced to learn the hard way. Not optional given the precedent.

**Status:** Open. Proposed 2026-08-10, off Drift-Bench's (#60) own measured results.
...
**Status:** Open. Proposed 2026-08-10. Structural/plumbing work — highest leverage if done before #68/#69 rather than after, lower urgency on its own.
