...
**Status:** ✅ Implemented (2026-08-24), with a real prerequisite bug found and fixed along the way.

- **Found first, fixed first: Feed Intelligence's own novelty/anomaly signals were silently broken against real data.** `feed_intelligence_router.py` converted `FeedRun.to_dict()` for `compute_feed_intelligence` without ever including a `citations` key — the real field is `citations_added` (a list of citation-id strings), so every count-based signal (`flag_anomalies`'s spike/drop/stale detection) read an always-empty list and never fired against a real feed. Fixed by passing `citations_added` through as `citations`, plus hardening `_count_topics` to skip non-dict entries (citation ids are bare strings, not resolved dicts with topic/title — that part of trending honestly stays uncomputable from run history alone, not silently pretended to work). New regression tests in `tests/test_feed_intelligence_router.py`.
- **Deterministic interval adjustment** — new `core/tropebook/adaptive_scheduling.py`: `recommend_interval_change()` reads a feed's own `results_count` history directly (already correctly tracked per-run, doesn't need the topic-analysis path at all) rather than routing through the now-fixed-but-still-topic-blind `feed_intelligence.py`. A streak of `LOW_NOVELTY_STREAK` (3) zero-novelty runs recommends lengthening one tier (daily→weekly→monthly, capped); a run whose `results_count` exceeds `SPIKE_MULTIPLIER` (3x) the recent average recommends shortening one tier (capped at daily). `"manual"`-interval feeds are never touched — an explicit scheduling opt-out this shouldn't override. Wired into `FeedScheduler.run_feed()` (called after every successful run, both scheduled and manually-triggered "Run Now"), fails open on its own errors so a bug here can't break the actual research run it runs after, and logs via `_emit_telemetry` so an automatic adjustment is visible, not silent.
- **Query rewrite is suggestion-only, on demand** — new `POST /api/research-feeds/{feed_id}/suggest-query-rewrite`. Gated on the same low-novelty-streak signal so it never calls the LLM unless the feed has actually gone stagnant (cost control, not spend-on-every-request). Never mutates the feed — apply a suggestion yourself via the existing `PUT /{feed_id}` endpoint, reusing infra rather than adding a second write path (#91's own citation-ids precedent). Kept deliberately separate from the automatic interval adjustment: interval changes are cheap/deterministic/bounded, a rewrite is generative and LLM-cost, matching #82's "on-demand trigger, not automatic" stance for anything LLM-driven and #19's cut of "suggest process improvements" as too generative to ship unbounded.

Tests: 26 new in `tests/test_adaptive_scheduling.py` (pure functions) + 3 new in `tests/test_research_feeds.py` (scheduler wiring, including a fail-open regression) + 5 new in `tests/test_feed_import_export.py` (router). Full suite: 2600 passing (was 2574). Live-verified the new endpoint's schema against the real server (404 on an unknown feed id); the LLM-calling path itself wasn't live-fired, matching this session's standing discipline against paying for real spend to re-prove what mocked tests already cover.
...
**Status:** ✅ Implemented (2026-08-24), with two real gaps found and worked around rather than assumed away.

- **Checked first: no per-citation platform data exists anywhere to group by.** BraveSearch-sourced citations only ever carry `source="brave"`/`"duckduckgo"` (`core/tropebook/research.py`). last30days' richer `[reddit]`/`[github]`/`[x]` platform tags exist only as unstructured inline text inside its raw report markdown — `run_query_and_extract_citations` (`core/last30days/runner.py`) only ever extracts `title`/`url` per citation, nothing platform-specific. Built around this instead of parsing fragile inline tags: new `core/tropebook/source_coverage.py`'s `classify_source_domain()` groups by the citation's own URL domain instead, which every citation has regardless of which engine produced it. Falls back to the bare registrable domain (not a generic "other" bucket) for anything unrecognized, so a long tail of one-off sources stays visible rather than collapsing into an undifferentiated pile.
- **Checked second: `Citation.access_count` is declared on the dataclass but never incremented anywhere in this codebase**, and `core/knowledge_decay.py`'s `score_citation` reads a `reference_count` field that doesn't exist on `Citation` at all — both permanently 0/absent for every real citation. Neither is a usable "was this actually useful" signal. Used `Decision.citation_ids` (#82/#91) instead: a citation referenced by one of this project's own decisions is a far more grounded "this mattered" signal than an uninstrumented counter.
- New `GET /api/memory/{project}/research/source-coverage` — citation pool is every citation referenced by this project's own decisions, unioned with every citation accumulated by a feed visible to this project (#86's `list_feeds(visible_to_project=...)`, shipped earlier this same session and a direct enabler here).
- **Disable is a display-time filter, not an ingestion-time block — disclosed, not silently narrowed.** `GET`/`PUT /api/memory/{project}/research/disabled-sources` manage a per-project suppression list, reflected in the coverage report's `disabled` flag. Deliberately does NOT stop a feed from ingesting a disabled source's citations: a feed can be visible to several projects at once (#86), ingestion happens once per feed not once per project, so an ingestion-time block would either suppress the source for every project sharing that feed or require per-project citation forking — both a materially bigger change than this entry's "disable per project" framing implied at face value.

Tests: 17 new in `tests/test_source_coverage.py` (pure functions) + 8 new in `tests/test_source_coverage_router.py` (router). Full suite: 2645 passing (was 2620). Live-verified against the real `tropelex` project: real domain classification across its actual citation pool (github/academic/blog/youtube/researchgate.net-as-fallback, correct counts), `useful_count: 0` everywhere since no real decision has populated `citation_ids` yet — an honest result, not a bug, given #91's citation-attach flow is brand new this session.
...
**Status:** Partially ✅ implemented 2026-08-31 — went further than the "minimum viable, cheap" tier originally scoped here. Corrected the causal record first: #97 (Uninstall Instructions) was re-attributed away from this item to its real cause, a separate realization about cross-machine export/import version clashes (see decision `05a00ddb3433`). That realization directly produced this work: `core/version.py` is now the single source of truth for `APP_VERSION` (was hardcoded in 4 independent places) and a new `MEMORY_SCHEMA_VERSION` (the schema-versioning discipline this entry's own text said didn't exist yet). `/api/account/import` now 409s on a schema_version mismatch (including a missing one) unless explicitly confirmed, instead of silently overwriting project files with zero validation, which is what it actually did before today. Benchmarks export/import gets the same `schema_version` field with a real mismatch diagnostic. README's new "## Versioning" section documents the policy. Still open: the moderate tier (a passive "update available" badge comparing against the latest GitHub release) and the large tier (real packaging) — both exactly as out-of-scope as this entry always said.
...
**Status:** ✅ Implemented 2026-09-01. `core/ghost/intent_check.py`'s `check_intent_conflict` — a single structured YES/NO-plus-rationale question via `core.llm.chat` (Ollama-first/OpenAI-fallback, no new LLM plumbing), fired only for an existing medium-severity `match_type == "keyword"` warning against a high/critical `risk_level` decision, escalating that warning to `high` with an attached `intent_check_rationale` on a real conflict — advisory only, no new block path. Cached (`memory["intent_check_cache"]`, keyed by decision+diff hash) and rate-limited (`memory["intent_check_rate_limit"]`, 50 calls/project/day default) in project memory, not in-process, so both survive a restart; every call — cached or fresh, conflict or not — logged to the audit trail as `ghost_intent_check`. Fails closed: any LLM response not starting with YES, including no backend being available at all, is treated as "no additional signal," never an implicit conflict. Live-verified against a real OpenAI backend (not mocked) on both a true-positive (payment-auth-removal diff) and a true-negative (unrelated docs diff) — see `docs/cais-summary.md`'s Evaluation & Limitations section for the exact transcript summary. Does not move Drift-Bench's `reward_hacking_keyword_evasion` or `multi_step_drift_gradual_removal` cells — both are built around zero-keyword-overlap diffs, so Ghost produces no warning there for #106 to escalate in the first place; confirmed by direct testing, not assumed. Wired into the real `ghost_check` endpoint (`core/ghost/preventive_router.py`), tested in `tests/test_ghost_intent_check.py` (18 tests, all mocking `core.llm.chat`, none hitting a real API in CI).
...
**Status:** ✅ Implemented (spike) 2026-09-01. `core/handoff/adapters/langgraph_adapter.py` — LangGraph, producer direction only, per this item's own "minimal producer OR consumer, not a full two-way integration" scoping. `generate_handoff_node` is a real LangGraph node (`async def node(state) -> dict`) that calls `core.handoff.router.generate_handoff` directly and in-process — the actual production router function, not a reimplementation — and `build_handoff_graph()` compiles a real `StateGraph` (`START -> generate_handoff_node -> END`). Live-verified with langgraph actually installed and exercised, not stubbed: `scripts/adapters/langgraph_handoff_demo.py` runs a real `.ainvoke()` end to end and prints the resulting packet (real `packet_hash`, real `must_survive_decision_ids`), and 4/4 tests in `tests/test_langgraph_adapter.py` pass against a real compiled graph. `langgraph` added as an optional extra (`pyproject.toml`'s `[project.optional-dependencies].adapters`), installed only into the isolated `uv`-managed `.venv` — the live app's system-Python environment is untouched, so this doesn't affect the main install or any shared dependency version. Tests `pytest.importorskip("langgraph")` and skip cleanly on the main dev environment (no extra installed there), matching #106's own no-backend-available degradation philosophy: honestly skip rather than fake a pass. Explicitly a spike, not general compatibility: one framework only (no AutoGen, no CrewAI), one direction only (producer — nothing here validates Tropelex consuming a LangGraph-native object). `docs/far-ai-summary.md`'s Deliverable #2 and Evaluation & Limitations section updated to describe exactly this scope, not more.
...
**2026-09-01 follow-up:** User noticed GUIDE results were landing too coarsely — 33 of the GUIDE's 77 `<h3>` sub-headings had no id of their own and fell back to their enclosing `<section>`'s anchor, so e.g. all 6 numbered "Getting Started" steps resolved to the same `/guide#getting-started`. `scripts/anchor_guide_subsections.py` retrofitted unique, collision-safe ids onto all 33 (idempotent, no `core/docs_search.py` changes needed — its ancestor-id fallback just had something more specific to find). Added a complementary JS fallback, `site/search-highlight.js` (shared across all 4 site pages + synced local mirrors): reads a `?hl=<query>` param on the target page, finds the first occurrence of the matched text (exact phrase, then per-token), highlights and scrolls to it — refining the anchor-level landing for matches that hit body text rather than a heading. Two real bugs found via live browser testing before this worked correctly, not assumed: the GUIDE's own sidebar TOC nav lists every section title *before* the real content in DOM order, so an unqualified text scan matched nav labels instead of prose; and Tailwind's CDN script injects styles asynchronously, shifting layout right after the page's initial scroll on a cold load — fixed with a nav-exclusion filter and a corrective re-scroll after a short delay, respectively. README.md stays plain markdown (user decision) — GitHub already gives it real per-heading anchors natively and can't run JS regardless of format, so the GUIDE remains the one actively-anchored, JS-enhanced surface.
