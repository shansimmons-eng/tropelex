...
- **Not yet a general-purpose MCP server.** These properties are currently reachable via Tropelex's own REST API and editor integrations (VSCode, Emacs, OpenCode). Exposing them as MCP tools — so any MCP-capable agent can query decision history, coordination packets, and calibration data directly — is a natural next step and is on the [wishlist](wishlist.md).
